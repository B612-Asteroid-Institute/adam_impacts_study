...
from conversions import impactor_to_adam_orbit, sorcha_output_to_df, sorcha_output_to_od_observations, fo_to_adam_orbit_cov

from fo_od import sorcha_df_to_fo_input, run_fo_od
...
def run_impact_study_fo(
    ...
    physical_params_list = [float(param) for param in sorcha_physical_params_string.split()]
    ...
    sorcha_observations_df = sorcha_output_to_df(f"{sorcha_output_name}/{sorcha_output_file}")
    ...
    for obj in object_ids:
        ...
        for day in unique_days:
            ...
            try:
                fo_orbit = run_fo_od(filtered_df, fo_file_name, fo_output_folder, FO_DIR, RUN_DIR, RESULT_DIR)
            except Exception as e:
                print(f"Error running find_orb output for {obj}: {e}")
                continue
            if len(fo_orbit) > 0:
                time = initial_orbit_objects.select("object_id", obj).coordinates.time[0]
                orbit = fo_orbit
                try:
                    result = propagator.propagate_orbits(orbit, time, covariance=True, num_samples=1000)
                    results, impacts = calculate_impacts(result, 60, propagator, num_samples=10000)
